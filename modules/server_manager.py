...
class ServerManagerView(disnake.ui.View):
    ...
    def build_embed(self, interaction: Union[disnake.MessageInteraction, CustomContext]):
        ...
        embed = disnake.Embed(
            ...
            description=f"```{self.current_guild.name}```\n"
                        ...
                        f"**オーナー:** `{self.current_guild.owner} [{self.current_guild.owner.id}]`\n"
                        f"**作成日:** <t:{created_at}:f> - <t:{created_at}:R>\n"
                        f"**参加日:** <t:{joined_at}:f> - <t:{joined_at}:R>\n"
                        f"**認証レベル:** `{self.current_guild.verification_level or 'なし'}`\n"
                        f"**メンバー数:** `{self.member_count(self.current_guild)}`\n"
                        f"**ボット数:** `{self.bot_count(self.current_guild)}`"
        )

        if len(self.pages) > 1:
            embed.title = f"現在のページ: [{self.current_page+1}/{len(self.pages)}]"
    ...
    def rebuild_components(self):
        ...
        if has_server_select and len(self.pages) > 1:
            ...
            back = disnake.ui.Button(label="戻る", emoji="⬅️")
            back.callback = self.previous_page
            self.add_item(back)

            next = disnake.ui.Button(label="進む", emoji="➡️")
            next.callback = self.next_page
            self.add_item(next)

        leave = disnake.ui.Button(label="退出", emoji="♻️", style=disnake.ButtonStyle.red)
        leave.callback = self.leave_guild
        self.add_item(leave)

        stop = disnake.ui.Button(label="停止", emoji="⏹️", style=disnake.ButtonStyle.blurple)
    ...
    async def interaction_check(self, interaction: disnake.MessageInteraction) -> bool:
        ...
        if interaction.author.id != self.inter.user.id:
            await interaction.response.send_message("ここでは操作できません...", ephemeral=True)
            return False
...
class ServerManagerCog(commands.Cog):
    ...
    @commands.command(name="servers", aliases=["servermanager"], hidden=True,
                      description="ボットが参加しているサーバーを管理します。")
