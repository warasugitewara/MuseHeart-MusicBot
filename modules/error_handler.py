...
class ErrorHandler(commands.Cog):
    ...
    async def on_legacy_command_error(self, ctx: CustomContext, error: Exception, resp_msg=True):
        ...
        if isinstance(error, commands.NotOwner):
            print(f"{ctx.author} [{ctx.author.id}] はボットのオーナーではないため、コマンドを使用できません: {ctx.command.name}")
            return
    ...
    async def on_error_report(self, inter: disnake.MessageInteraction):
        ...
        if str(inter.author.id) not in inter.message.content:
            await inter.send(f"{inter.message.content} のユーザーのみがこのボタンを使用できます！", ephemeral=True)
            return
    ...
    async def on_report_submit(self, inter: disnake.ModalInteraction):
        ...
        embed.add_field(name="ログ：", value=inter.message.embeds[0].description)

        await inter.response.edit_message(
            embed=disnake.Embed(
                description="**エラーが正常に報告されました！**",
                color=self.bot.get_color(inter.guild.me)
            ), view=None
        )
        ...
        embed.set_author(name=f"エラー報告者: {inter.author} - {inter.author.id}", icon_url=user_avatar)

        guild_txt = f"サーバー: {inter.guild.name} [{inter.guild.id}]"
    ...
    def build_report_embed(self, ctx):
        ...
        if ctx.guild:
            ...
            if vcbot := ctx.guild.me.voice:
                if vc and vcbot.channel != vc.channel:
                    embed.add_field(
                        name="ボイスチャンネル（ボット）：", inline=False,
        ...
        try:
            ...
            embed.description = f"**スラッシュコマンド：**```\n{ctx.data.name}``` "

            if ctx.filled_options:
                embed.description += "**オプション**```\n" + \
