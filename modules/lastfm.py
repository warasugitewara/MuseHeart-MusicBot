...
class LastFMView(disnake.ui.View):
    ...
    def build_components(self):
        ...
        if self.session_key:
            ...
            btn1 = disnake.ui.Button(label=f"スクロブル/記録を{'無効' if self.scrobble_enabled else '有効'}にする")
            btn1.callback = self.scrobble_enable
            self.add_item(btn1)

            btn2 = disnake.ui.Button(label="Last.fmアカウントの連携を解除", style=disnake.ButtonStyle.red)
            btn2.callback = self.disconnect_account
            self.add_item(btn2)

        else:
            btn3 = disnake.ui.Button(label="Last.fmアカウントを連携")
    ...
    async def scrobble_enable(self, interaction: disnake.MessageInteraction):
        ...
        if (retry_after := self.cooldown_scrobble_btn.get_bucket(interaction).update_rate_limit()):
            await interaction.send(f"**スクロブル/記録の有効/無効を切り替えるには、あと{int(retry_after)}秒お待ちください。**", ephemeral=True)
            return
    ...
    async def send_authurl_callback(self, interaction: disnake.MessageInteraction):
        ...
        await interaction.send(f"### [こちらをクリック](<{self.auth_url}>)してLast.fmアカウントを連携してください（ページで「allow」をクリック）\n\n"
                               f"`リンクの有効期限:` <t:{self.last_timestamp}:R> `（期限切れの場合は、もう一度ボタンをクリックしてください）`\n\n"
                               f"`注意: 「こちらをクリック」のリンクを他の人に見せたり、公開の場所に送信しないでください。"
                               f"このリンクを使用すると、あなたのLast.fmアカウントにアクセスできてしまいます。`\n\n"
                               "`アプリケーションをすでに承認している場合は、上記のメッセージが更新されて処理が"
                               "確認されるまで最大20秒お待ちください。`",
...
class LastFmCog(commands.Cog):
    ...
    @commands.command(hidden=True, name="lastfm", aliases=["lastfmconnect", "lfm"],
                      description="Last.fmアカウントを連携してスクロブルで楽曲を記録します。",
    ...
    @commands.slash_command(hidden=True, name="lastfm",
                      description=f"{desc_prefix}Last.fmアカウントを連携してスクロブルで楽曲を記録します。",
    ...
    async def lastfm(self, inter: disnake.ApplicationCommandInteraction):
        ...
        try:
            if not inter.permissions.embed_links:
                raise GenericError(f"**チャンネル <#{inter.channel_id}> でリンク/添付ファイルを挿入する権限がありません**")
        ...
        if lastfm_user:
            ...
            txt = f"> `👤` **⠂ユーザー:** [`{name}`](<{lastfm_user['url']}>)\n" \
                  f"> `⏰` **⠂アカウント作成日:** <t:{lastfm_user['registered']['#text']}:f>\n"

            if lastfm_user['country']:
                txt += f"> `🌎` **⠂国:** `{lastfm_user['country']}`\n"

            if playcount := lastfm_user['playcount']:
                txt += f"> `🔊` **⠂総再生曲数:** [`{int(playcount):,}`](<https://www.last.fm/user/{lastfm_user['name']}/library>)\n"

            if playlists := lastfm_user['playlists'] != "0":
                txt += f"> `📄` **⠂公開プレイリスト:** [`{int(playlists):,}`](<https://www.last.fm/user/{lastfm_user['name']}/playlists>)\n"
            ...
            try:
                ...
                if top_tracks:
                    ...
                    embed = disnake.Embed(
                        description="\n".join(f"> ` {n+1}位 ` [`{t['name']}`]({t['url']}) `アーティスト:` [`{t['artist']['name']}`]({t['artist']['url']}) `(x{int(t['playcount']):,})`" for n, t in enumerate(top_tracks)),
                        color=embed_color).set_author(name=f"トップ3: 最も再生した楽曲（全{int(lastfm_user['track_count']):,}曲中）:",
                ...
                if top_artists:
                    ...
                    embed = disnake.Embed(
                        description="\n".join(f"> ` {n+1}位 ` [`{t['name']}`]({t['url']}) `(x{int(t['playcount']):,})`" for n, t in enumerate(top_artists)),
                        color=embed_color).set_author(name=f"トップ3: 最も再生したアーティスト（全{int(lastfm_user['artist_count']):,}組中）:",
                ...
                if top_albuns:
                    ...
                    embed = disnake.Embed(
                        description="\n".join(f"> ` {n+1}位 ` [`{b['name']}`]({b['url']}) `アーティスト:` [`{b['artist']['name']}`]({b['artist']['url']}) `(x{int(b['playcount']):,})`" for n, b in enumerate(top_albuns)),
                        color=embed_color).set_author(name=f"トップ3: 最も再生したアルバム（全{int(lastfm_user['album_count']):,}枚中）:",
            ...
            embeds = [disnake.Embed(
                description="**[Last.fm](<https://www.last.fm/home>)のアカウントを連携（または作成）すると、"
                            "ここで聴いたすべての楽曲がLast.fmプロフィールに記録されます。楽曲/アーティスト/アルバムの"
                            "おすすめを受け取ったり、聴いた音楽の統計情報を確認したり、プラットフォームの素晴らしい"
                            "コミュニティにアクセスできます。**",
                color=embed_color
        ...
        if view.session_key:
            ...
            embeds[0].description += f"\n### アカウント [{view.username}](<https://www.last.fm/user/{view.username}>) が" \
                                 "正常に連携されました！\n\n`これからボイスチャンネルで音楽を聴くと、" \
                                "自動的にLast.fmアカウントに記録されます`"
        ...
        if not (album := track.album_name) and track.info["sourceName"] in ("youtube", "soundcloud"):
            ...
            if track.ytid and track.author.endswith(" - topic") and not track.author.endswith("Release - topic") and not track.title.startswith(track.author[:-8]):
                ...
                if (fmdata := self.bot.last_fm.cache.get(track_query)) is None:
                    ...
                    if not result:
                        print(f"⚠️ - Last.FM Scrobble - 楽曲の検索結果がありません: {track_query}")
                ...
                if not fmdata:
                    print(f"⚠️ - Last.FM Scrobble - スキップしました: {track_query}")
                    await self.save_scrobble(query=track_query, track=track, users=users)
                    return
...
def setup(bot):
    if not bot.pool.config["LASTFM_KEY"] or not bot.pool.config["LASTFM_SECRET"]:
        print(("="*48) + "\n⚠️ - LASTFM_KEYとLASTFM_SECRETが設定されていないため、Last.FM機能は無効になります")
        return
